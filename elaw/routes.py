...
@app.route('/court_manage_laws')
def court_manage_laws():
    m=Law.query.all()
    return render_template("court_manage_laws.html",m=m)

@app.route('/court_manage_announcement')
def court_manage_announcement():
    a=announcement.query.all()
    return render_template("court_manage_announcement.html",a=a)


@app.route('/court_manage_rules')
def court_manage_rules():
    a=rules.query.all()
    return render_template("court_manage_rules.html",a=a)


@app.route('/court_manage_caserecords')
def court_manage_caserecords():
    a=caserecords.query.all()
    return render_template("court_manage_caserecords.html",a=a)

@app.route('/court_manage_jurydetails')
def court_manage_jurydetails():
    a=jurys.query.all()
    return render_template("court_manage_jurydetails.html",a=a)
...
@app.route('/court_index/<id>')
def court_index(id):
    return render_template("court_index.html")
...
@app.route('/court_add_law', methods = ['GET','POST'])
def court_add_law():
    if request.method == 'POST':
        law = request.form['law']
        details = request.form['details']
        ...
        db.session.add(my_data) 
        db.session.commit()
        return redirect('/court_manage_laws')
    else :
        return render_template("court_add_law.html")

@app.route('/court_add_announcement', methods = ['GET','POST'])
def court_add_announcement():
    if request.method == 'POST':
        ann = request.form['announcement']
        details = request.form['details']
        my_data = announcement(ann=ann, details=details)
        db.session.add(my_data) 
        db.session.commit()
        return redirect('/court_manage_announcement')
    else :
        return render_template("court_add_announcement.html")

@app.route('/court_add_rule', methods = ['GET','POST'])
def court_add_rule():
    if request.method == 'POST':
        rule = request.form['rule']
        details = request.form['details']
        my_data = rules(rules=rule, details=details)
        db.session.add(my_data) 
        db.session.commit()
        return redirect('/court_manage_rules')
    else :
        return render_template("court_add_rule.html")

@app.route('/court_add_caserecord', methods = ['GET','POST'])
def court_add_record():
    if request.method == 'POST':
        case = request.form['caserecord']
        disc = request.form['details']
        my_data = caserecords(records=case, description=disc)
        db.session.add(my_data) 
        db.session.commit()
        return redirect('/court_manage_caserecords')
    else :
        return render_template("court_add_caserecord.html")
...
@app.route('/court_add_jurydetails', methods = ['GET','POST'])
def court_add_jury():
    if request.method == 'POST':
        jury = request.form['jn']
        details = request.form['details']
        qualification = request.form['qual']
        experience = request.form['exp']
        specialization = request.form['special']
        court = request.form['court']
        my_data = jurys(juryname=jury, details=details,qualification=qualification,experience=experience,specialization=specialization,court=court)
        db.session.add(my_data) 
        db.session.commit()
        return redirect('/court_manage_jurydetails')
    else :
        return render_template("court_add_jurydetails.html")
...
def edit_lawyerprofile(id):
    d = Login.query.get_or_404(id)
    m=Login.query.filter_by(usertype='court').all()
    if request.method == 'POST':
        d.name = request.form['name']
        d.contact = request.form['contact']
        d.username = request.form['username']
...
@app.route('/court_edit_law/<int:id>', methods = ['GET','POST'])
def court_edit_law(id):
    d = Law.query.get_or_404(id)
    if request.method == 'POST':
        d.law = request.form['law']
        d.details = request.form['details']
        ...
        return render_template("court_edit_law.html",d=d)

@app.route('/court_edit_rule/<int:id>', methods = ['GET','POST'])
def court_edit_rule(id):
    d = rules.query.get_or_404(id)
    if request.method == 'POST':
        d.rules = request.form['rule']
        d.details = request.form['details']
        db.session.commit()
        return redirect('/court_manage_rules')
    else :
        return render_template("court_edit_rule.html",d=d)


@app.route('/court_edit_announcement/<int:id>', methods = ['GET','POST'])
def court_edit_announcement(id):
    d = announcement.query.get_or_404(id)
    if request.method == 'POST':
        d.ann = request.form['ann']
        d.details = request.form['details']
        db.session.commit()
        return redirect('/court_manage_announcement')
    else :
        return render_template("court_edit_announcement.html",d=d)

@app.route('/court_edit_caserecord/<int:id>', methods = ['GET','POST'])
def court_edit_caserecord(id):
    d = caserecords.query.get_or_404(id)
    if request.method == 'POST':
        d.records = request.form['record']
        d.description = request.form['details']
        db.session.commit()
        return redirect('/court_manage_caserecords')
    else :
        return render_template("court_edit_caserecord.html",d=d)

@app.route('/court_edit_jurydetails/<int:id>', methods = ['GET','POST'])
def court_edit_jurydetails(id):
    d = jurys.query.get_or_404(id)
    if request.method == 'POST':
        d.juryname = request.form['jn']
        d.details = request.form['details']

        d.qualification = request.form['qual']
        d.experience = request.form['exp']
        d.specialization = request.form['special']
        d.court = request.form['court']
        
        db.session.commit()
        return redirect('/court_manage_jurydetails')
    else :
        return render_template("court_edit_jurydetails.html",d=d)
...
def delete_announcement(id):
    delet = announcement.query.get_or_404(id)
...
@app.route('/delete_rule/<int:id>', methods = ['GET','POST'])
@login_required
def delete_rule(id):
    delet = rules.query.get_or_404(id)
...
@app.route('/delete_law_by_court/<int:id>', methods = ['GET','POST'])
@login_required
def delete_law_by_court(id):
    delet = Law.query.get_or_404(id)
...
@app.route('/delete_caserecord/<int:id>', methods = ['GET','POST'])
@login_required
def delete_caserecord(id):
    delet = caserecords.query.get_or_404(id)
...
@app.route('/court_delete_jurydetails/<int:id>', methods = ['GET','POST'])
@login_required
def court_delete_jurydetails(id):
    delet = jurys.query.get_or_404(id)
...
def login():
     if request.method=="POST":
         ...
         court=Login.query.filter_by(username=username,password=password, usertype= 'court').first()
         ...
         if admin:
             ...
             login_user(court)
             next_page = request.args.get('next')
             return redirect(next_page) if next_page else redirect('/court_index/'+str(court.id))
...
@app.route('/court_register',methods=['GET', 'POST'])
def court_register():
    if request.method == 'POST':
        name = request.form['name']
        contact = request.form['contact']
        ...
        my_data =Login(name=name,contact=contact,location=location,type=type,address=address,username=email,password=password,usertype="court")
    #     try:
    ...
    #         sendmail(email)
    #     except:
    #         r="Sorry, Your Registeration Failed.Please Register with Valid Details."
    #         return render_template("court_register.html",r=r)
            
    # else :
    #     n="Your Registeration will be confirmed soon.."
    #     return render_template("court_register.html",n=n)
        db.session.add(my_data) 
        db.session.commit()
        flash("Registered successfully! Please Login..")
        return redirect('/login')
...
def lawyer_register():
    d=Login.query.filter_by(usertype='court').all()
    if request.method == 'POST':
        name = request.form['name']
        email = request.form['email']
        contact = request.form['contact']
...
@login_required
@app.route('/user_view_court',methods=["GET","POST"])
def user_view_court():
    obj = Login.query.filter_by(usertype="court").all()
    return render_template("user_view_court.html",obj=obj)

@login_required
@app.route('/admin_view_court',methods=["GET","POST"])
def admin_view_court():
    obj = Login.query.filter_by(usertype="court").all()
    return render_template("admin_view_court.html",obj=obj)

@login_required
@app.route('/lawyer_view_court',methods=["GET","POST"])
def lawyer_view_court():
    obj = Login.query.filter_by(usertype="court").all()
    return render_template("lawyer_view_court.html",obj=obj)
...
@login_required
@app.route('/user_view_announcements',methods=["GET","POST"])
def user_view_announcements():
    obj = announcement.query.all()
    return render_template("user_view_announcements.html",obj=obj)

@app.route('/user_view_rules',methods=["GET","POST"])
def user_view_rules():
    obj = rules.query.all()
    return render_template("user_view_rules.html",obj=obj)

@app.route('/user_view_case_records',methods=["GET","POST"])
def user_view_case_records():
    obj = caserecords.query.all()
    return render_template("user_view_case_records.html",obj=obj)

@app.route('/user_view_jurys',methods=["GET","POST"])
def user_view_jurys():
    obj = jurys.query.all()
    return render_template("user_view_jurys.html",obj=obj)
...
@login_required
@app.route('/lawyer_view_announcements',methods=["GET","POST"])
def lawyer_view_announcements():
    obj = announcement.query.all()
    return render_template("lawyer_view_announcement.html",obj=obj)

@app.route('/lawyer_view_rules',methods=["GET","POST"])
def lawyer_view_rules():
    obj = rules.query.all()
    return render_template("user_view_rules.html",obj=obj)

@app.route('/lawyer_view_case_records',methods=["GET","POST"])
def lawyer_view_case_records():
    obj = caserecords.query.all()
    return render_template("lawyer_view_case_records.html",obj=obj)

@app.route('/lawyer_view_jurys',methods=["GET","POST"])
def lawyer_view_jurys():
    obj = jurys.query.all()
    return render_template("user_view_jurys.html",obj=obj)
...
@login_required
@app.route('/Court_contact/<id>', methods = ['GET','POST'])
def court_contact(id):
    d=Login.query.filter_by(id=id).first()
    ...
    if request.method == 'POST':
        ...
        subject = request.form['subject']
        message = request.form['message']
        my_data = Contact(name=name, email=email,contact=contact,subject=subject,message=message,usertype="court")
        db.session.add(my_data) 
        db.session.commit()
   
        return redirect('/court_index/'+str(current_user.id))
    else :
        return render_template("court_contact.html",d=d)
